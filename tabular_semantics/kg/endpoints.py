...
class SPARQLEndpoint(object):
    ...
    def __init__(self, endpoint_url, user_agent=None):
        '''
        Constructor
        '''
        if user_agent:
            self.sparqlw = SPARQLWrapper(endpoint_url, agent=user_agent)
        else:
            self.sparqlw = SPARQLWrapper(endpoint_url)
        self.sparqlw.setReturnFormat(JSON)
        self.sparqlw.setTimeout(500)

    def createSPARQLQuerySameAsEntities(self, ent):
    ...
    def createSPARQLQuerySameAsEntitiesMulti(self, ents):
        raise NotImplementedError

    def getSameEntitiesMulti(self, ents):
        query = self.createSPARQLQuerySameAsEntitiesMulti(ents)
        return self.getQueryResultsArityTwo(query)

    def createSPARQLQueryRedirectEntities(self, ent):
        raise NotImplementedError

    def getRedirectEntities(self, ent):
        query = self.createSPARQLQueryRedirectEntities(ent)
        return self.getQueryResultsArityOne(query)

    def createSPARQLQueryRedirectEntitiesMulti(self, ents):
        raise NotImplementedError

    def getRedirectEntitiesMulti(self, ents):
        query = self.createSPARQLQueryRedirectEntitiesMulti(ents)
    ...
    def createSPARQLEntitiesForClass(self, cls, offset, limit):
        raise NotImplementedError
    ...
    def createSPARQLEntitiesLabelsForClass(self, cls, offset, limit):
        raise NotImplementedError
    ...
    def createSPARQLQueryTypesForSubject(self, entity):
        raise NotImplementedError
    ...
    def createSPARQLQueryAllTypesForSubject(self, entity):
        raise NotImplementedError
    ...
    def createSPARQLQueryAllTypesForSubjectMulti(self, entities):
        raise NotImplementedError

    def getAllTypesForEntityMulti(self, entities):
        query = self.createSPARQLQueryAllTypesForSubjectMulti(entities)
        return self.getQueryResultsArityTwo(query)
    ...
    def createSPARQLQueryAllSuperClassesForClass(self, uri_class):
        raise NotImplementedError
    ...
    def createSPARQLQueryAllSuperClassesForClassMulti(self, uri_classes):
        raise NotImplementedError

    def getAllSuperClassesMulti(self, uri_classes):
        query = self.createSPARQLQueryAllSuperClassesForClassMulti(uri_classes)
        return self.getQueryResultsArityTwo(query)

    def createSPARQLQueryDistanceToAllSuperClassesForClass(self, uri_class):
        raise NotImplementedError
    ...
    def createSPARQLQueryAllSubClassesForClass(self, uri_class):
        raise NotImplementedError
    ...
    def createSPARQLQueryAllSubClassesForClassMulti(self, uri_classes):
        raise NotImplementedError

    def getAllSubClassesMulti(self, uri_classes):
        query = self.createSPARQLQueryAllSubClassesForClassMulti(uri_classes)
        return self.getQueryResultsArityTwo(query)

    def createSPARQLQueryDistanceToAllSubClassesForClass(self, uri_class):
        raise NotImplementedError
    ...
    def createSPARQLQueryDomainTypesCountOfPredicatesForSubject(self, subject, limit=3):
        return "SELECT DISTINCT ?uri WHERE { <" + subject + "> ?p [] . ?p rdfs:domain ?uri . } GROUP BY ?uri HAVING (COUNT(?uri) > 3) ORDER BY DESC(COUNT(?uri)) limit " + str(
            limit)
    ...
    def createSPARQLQueryRangeTypesCountOfPredicatesForObject(self, obj, limit=3):
        return "SELECT DISTINCT ?uri WHERE { [] ?p <" + obj + "> . ?p rdfs:range ?uri . } GROUP BY ?uri HAVING (COUNT(?uri) > 3) ORDER BY DESC(COUNT(?uri)) limit " + str(
            limit)
    ...
    def createSPARQLQueryTriplesForSubject(self, subject, limit=1000):
        ...
        return "SELECT DISTINCT ?outA ?outB WHERE { <" + subject + "> ?outA ?outB  FILTER( ?outA NOT IN(" + props_to_filter + ")) } limit " + str(
            limit)
    ...
    def createSPARQLQueryTriplesForObject(self, obj, limit=1000):
        ...
        return "SELECT DISTINCT ?outA ?outB WHERE { ?outA ?outB <" + obj + "> . FILTER( ?outB NOT IN(" + props_to_filter + ")) } limit " + str(
            limit)
    ...
    def getQueryResultsArityOne(self, query, filter_uri=True):
        ...
        for result in results["results"]["bindings"]:
            ...
            if not filter_uri or uri_value.startswith(URI_KG.dbpedia_uri) or uri_value.startswith(
                    URI_KG.wikidata_uri) or uri_value.startswith(URI_KG.schema_uri) or uri_value.startswith(
                    URI_KG.dbpedia_uri_resource) or uri_value.startswith(URI_KG.dbpedia_uri_property):
    ...
    def getQueryResultsArityTwo(self, query, filter_outA=True, filter_outB=True):
        ...
        for result in results["results"]["bindings"]:
            ...
            if not filter_outA or outA_value.startswith(URI_KG.dbpedia_uri) or outA_value.startswith(
                    URI_KG.wikidata_uri) or outA_value.startswith(URI_KG.schema_uri) or outA_value.startswith(
                    URI_KG.dbpedia_uri_resource) or outA_value.startswith(URI_KG.dbpedia_uri_property):

                if not filter_outB or outB_value.startswith(URI_KG.dbpedia_uri) or outB_value.startswith(
                        URI_KG.wikidata_uri) or outB_value.startswith(URI_KG.schema_uri) or outB_value.startswith(
                        URI_KG.dbpedia_uri_resource) or outB_value.startswith(URI_KG.dbpedia_uri_property):
...
class DBpediaEndpoint(SPARQLEndpoint):
    ...
    def createSPARQLEntitiesForClass(self, class_uri, offset=0, limit=1000):
        return "SELECT DISTINCT ?uri WHERE { ?uri <http://www.w3.org/1999/02/22-rdf-syntax-ns#type> <" + class_uri + "> . } ORDER BY RAND() OFFSET " + str(
    ...
    def createSPARQLEntitiesLabelsForClass(self, class_uri, offset=0, limit=1000):
        return "SELECT DISTINCT ?outA ?outB WHERE { ?outA <http://www.w3.org/1999/02/22-rdf-syntax-ns#type> <" + class_uri + "> . ?outA rdfs:label ?outB . FILTER( langMatches(lang(?outB), 'en')) } ORDER BY RAND() OFFSET " + str(
            offset) + " limit " + str(limit)
...
class WikidataEndpoint(SPARQLEndpoint):
    ...
    def __init__(self):
        ...
        Constructor
        '''
        super().__init__(self.getEndpoint(), "WDQS-example Python/%s.%s" % (sys.version_info[0], sys.version_info[1]))
        # "http://dbpedia.org/sparql"
    ...
    def createSPARQLEntitiesForClass(self, class_uri, offset=0, limit=1000):
        ...
        return "SELECT DISTINCT ?uri WHERE { ?uri <http://www.wikidata.org/prop/direct/P31> <" + \
               class_uri + "> . } ORDER BY RAND() OFFSET " + str(offset) + " limit " + str(limit)
    ...
    def createSPARQLQueryAllTypesForSubjectMulti(self, entities):
        return "SELECT DISTINCT (?entity as ?outA) (?uri as ?outB)" \
               + "WHERE {" \
               + "VALUES ?entity { " + " ".join(map(lambda x: "<" + x + ">", entities)) + " }" \
               + "{ ?entity <http://www.wikidata.org/prop/direct/P31> ?dt . " \
               + "?dt <http://www.wikidata.org/prop/direct/P279>* ?uri " \
               + "} " \
               + "UNION {" \
               + "?entity <http://www.wikidata.org/prop/direct/P31> ?uri . }" \
               + "}"
    ...
    def createSPARQLQuerySameAsEntitiesMulti(self, ents):
        return "SELECT DISTINCT (?entity as ?outA) (?uri as ?outB) " \
               + "WHERE {" \
               + "VALUES ?entity { " + " ".join(map(lambda x: "<" + x + ">", ents)) + " }" \
               + "{?entity <http://www.wikidata.org/prop/direct/P460> ?uri ." \
               + "} " \
               + "UNION {" \
               + "?uri <http://www.wikidata.org/prop/direct/P460> ?entity ." \
               + "}" \
               + "}"

    def createSPARQLQueryRedirectEntities(self, uri_entity):
        return "SELECT DISTINCT ?uri " \
               + "WHERE {" \
               + "{<" + uri_entity + "> <http://www.w3.org/2002/07/owl#sameAs> ?uri . }" \
               + "UNION" \
    ...
    def createSPARQLQueryRedirectEntitiesMulti(self, uri_entities):
        return "SELECT DISTINCT (?entity as ?outA) (?uri as ?outB) " \
               + "WHERE {" \
               + "VALUES ?entity { " + " ".join(map(lambda x: "<" + x + ">", uri_entities)) + " }" \
               + "{ ?entity <http://www.w3.org/2002/07/owl#sameAs> ?uri . }" \
               + "UNION" \
               + "{?uri <http://www.w3.org/2002/07/owl#sameAs> ?entity .}" \
               + "}"
    ...
    def createSPARQLQueryAllSuperClassesForClassMulti(self, uri_classes):
        return "SELECT DISTINCT (?class as ?outA) (?superclass as ?outB) " \
               + "WHERE {" \
               + "VALUES ?class { " + " ".join(map(lambda x: "<" + x + ">", uri_classes)) + " }" \
    ...
    def createSPARQLQueryAllSubClassesForClassMulti(self, uri_classes):
        return "SELECT DISTINCT (?class as ?outA) (?uri as ?outB) " \
               + "WHERE {" \
               + "VALUES ?class { " + " ".join(map(lambda x: "<" + x + ">", uri_classes)) + " }" \
               + "?uri <http://www.wikidata.org/prop/direct/P279>+ ?class" \
               + "}"
